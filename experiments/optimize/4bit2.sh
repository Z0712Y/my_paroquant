...
#============guidedquant：新增saliency相关配置变量=====
# 是否启用 GuidedQuant Saliency-Weighted Loss
USE_SALIENCY="是"
# saliency 提取时的 batch size（控制显存，建议 1~4）
SALIENCY_BATCH_SIZE=2
# saliency 归一化方式: mean / max / none
SALIENCY_NORM="mean"
# 预计算 saliency 文件路径（空字符串表示自动提取）
SALIENCY_PATH=""
#============guidedquant：saliency配置变量结束=====
...
#============guidedquant：在运行配置输出中增加saliency信息=====
【GuidedQuant Saliency】
  Use Saliency:  $USE_SALIENCY
  Saliency BS:   $SALIENCY_BATCH_SIZE
  Saliency Norm: $SALIENCY_NORM
#============guidedquant：saliency运行配置输出结束=====

#============guidedquant：在日志中增加saliency配置信息=====
【GuidedQuant Saliency】
  Use Saliency:  $USE_SALIENCY
  Saliency BS:   $SALIENCY_BATCH_SIZE
  Saliency Norm: $SALIENCY_NORM
  Saliency Path: ${SALIENCY_PATH:-"自动提取"}
#============guidedquant：saliency日志信息结束=====
...
#============guidedquant：构建saliency相关参数=====
SALIENCY_ARG=""
SALIENCY_BS_ARG=""
SALIENCY_NORM_ARG=""
SALIENCY_PATH_ARG=""
if [ "$USE_SALIENCY" = "是" ]; then
    SALIENCY_ARG="--use-saliency"
    SALIENCY_BS_ARG="--saliency-batch-size $SALIENCY_BATCH_SIZE"
    SALIENCY_NORM_ARG="--saliency-norm $SALIENCY_NORM"
    if [ -n "$SALIENCY_PATH" ]; then
        SALIENCY_PATH_ARG="--saliency-path $SALIENCY_PATH"
    fi
fi
#============guidedquant：saliency参数构建结束=====
