...
# Check if local GPQA dataset exists
_LOCAL_GPQA_PATH = os.path.join(os.path.dirname(__file__), "..", "local_datasets", "gpqa_diamond")
if os.path.exists(_LOCAL_GPQA_PATH):
    print(f"[INFO] Using local GPQA dataset from {_LOCAL_GPQA_PATH}")
    _gpqa_repo = _LOCAL_GPQA_PATH
    _gpqa_subset = None
    _gpqa_trust = False
else:
    _gpqa_repo = "Idavidrein/gpqa"
    _gpqa_subset = "gpqa_diamond"
    _gpqa_trust = True
