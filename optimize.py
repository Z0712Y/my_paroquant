...
from paroquant.intactkv_utils import compute_pivot_token_mask
#============intactKV：导入 intactKV 工具=====
...
class Config:
    ...
    #============intactKV：新增配置参数=====
    # 是否启用 Pivot-Aware 逐层校准损失
    pivot_aware_loss: bool = False
    # Pivot-Aware 损失的权重系数
    pivot_loss_weight: float = 2.0
    # Pivot token 识别模式: "prefix" | "bos" | "auto"
    pivot_mode: str = "prefix"
    # "prefix" 模式下前多少个 token 作为 pivot
    pivot_len: int = 16
    #============intactKV：新增配置参数=====
...
def main():
    ...
    #============intactKV：计算 Pivot Token Mask=====
    pivot_token_mask = None
    pivot_token_val_mask = None
    if args.pivot_aware_loss:
        logger.info("Computing pivot token masks...")
        with torch.no_grad():
            pivot_token_mask = compute_pivot_token_mask(
                samples, tokenizer, pivot_mode=args.pivot_mode, pivot_len=args.pivot_len
            )
            pivot_token_val_mask = compute_pivot_token_mask(
                val_samples, tokenizer, pivot_mode=args.pivot_mode, pivot_len=args.pivot_len
            )
            # 将 mask 也按 batch 分片，与 input_batches 对齐
            if args.batch_size is not None and args.batch_size > 0:
                num_batches = samples.shape[0] // args.batch_size
                pivot_token_mask = pivot_token_mask.chunk(num_batches)
            else:
                pivot_token_mask = [pivot_token_mask]
            if val_batch_size is not None and val_batch_size > 0:
                num_val_batches = val_samples.shape[0] // val_batch_size
                pivot_token_val_mask = pivot_token_val_mask.chunk(num_val_batches)
            else:
                pivot_token_val_mask = [pivot_token_val_mask]
    #============intactKV：计算 Pivot Token Mask=====
    ...
    for layer_idx, layer in enumerate(tqdm(blocks)):
        ...
        if not all_files_exist:
            ...
            for step, step_params_dict in enumerate(params_to_optimize):
                ...
                #============intactKV：传递 pivot_token_mask 和 pivot_loss_weight=====
                # 准备与当前层输入 batch 对齐的 pivot mask
                current_pivot_mask = None
                current_pivot_val_mask = None
                if args.pivot_aware_loss and pivot_token_mask is not None:
                    current_pivot_mask = pivot_token_mask
                    current_pivot_val_mask = pivot_token_val_mask
                ...
                optimize_module(
                    ...
                    pivot_token_mask=current_pivot_mask,
                    pivot_loss_weight=args.pivot_loss_weight if args.pivot_aware_loss else 0.0,
                )
                #============intactKV：传递 pivot_token_mask 和 pivot_loss_weight=====
