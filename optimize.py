...
#============guidedquant：新增saliency模块导入，用于提取/加载/保存saliency=====
from paroquant.saliency import (
    extract_layer_saliency,
    save_saliency,
    load_saliency,
)
#============guidedquant：saliency模块导入结束=====
...
class Config:
    ...
    #============guidedquant：新增saliency相关配置字段=====
    # 是否启用 saliency-weighted loss（GuidedQuant End Loss Guidance 思想）
    use_saliency: bool = False
    # 预计算 saliency 文件的加载路径（若提供则跳过提取）
    saliency_path: Optional[str] = None
    # saliency 提取时的 batch size（控制显存，建议 1~4）
    saliency_batch_size: int = 1
    # saliency 提取后保存的路径（默认 output_dir / saliency.pt）
    saliency_save_path: Optional[str] = None
    # saliency 归一化方式: "mean" | "max" | "none"
    saliency_norm: Literal["mean", "max", "none"] = "mean"
    #============guidedquant：saliency配置字段结束=====
...
def main():
    ...
    #============guidedquant：新增saliency提取或加载逻辑（在逐层量化之前执行）=====
    layer_saliency: Optional[dict] = None
    if args.use_saliency:
        saliency_file = (
            Path(args.saliency_path)
            if args.saliency_path
            else output_dir / "saliency.pt"
        )

        if args.saliency_path and Path(args.saliency_path).exists():
            layer_saliency = load_saliency(args.saliency_path, device=device)
        elif saliency_file.exists():
            layer_saliency = load_saliency(str(saliency_file), device=device)
        else:
            logger.info(
                "Extracting saliency via full-model forward+backward "
                f"(batch_size={args.saliency_batch_size})..."
            )
            # 使用较少的样本提取 saliency（512 个足够，节省时间和显存）
            saliency_samples = samples[: min(samples.shape[0], 512)]
            layer_saliency = extract_layer_saliency(
                model=model,
                samples=saliency_samples,
                batch_size=args.saliency_batch_size,
                device=device,
            )
            save_path = args.saliency_save_path or str(output_dir / "saliency.pt")
            save_saliency(layer_saliency, save_path)
            # extract_layer_saliency 最后会 model.cpu()，需要重新把 embed 移回 GPU
            move_embed(model, device)

        logger.info(f"Saliency loaded/extracted for {len(layer_saliency)} layers.")
    #============guidedquant：saliency提取/加载逻辑结束=====
    ...
    for layer_idx, layer in enumerate(tqdm(blocks)):
        ...
        #============guidedquant：新增：获取当前层的saliency向量=====
        current_saliency = None
        if args.use_saliency and layer_saliency is not None:
            current_saliency = layer_saliency.get(layer_idx)
            if current_saliency is None:
                logger.warning(f"No saliency found for layer {layer_idx}, falling back to uniform.")
        #============guidedquant：当前层saliency获取结束=====
        ...
        if not all_files_exist:
            ...
            for step, step_params_dict in enumerate(params_to_optimize):
                ...
                #============guidedquant：根据是否启用saliency动态选择loss_fn并传入saliency_weights=====
                if args.use_saliency and current_saliency is not None:
                    effective_loss_fn = "saliency_weighted_mse"
                else:
                    effective_loss_fn = args.loss
                ...
                optimize_module(
                    ...
                    early_stop=None,
                    saliency_weights=current_saliency,
                    post_optim_callback=reset_angles_by_mask,
                )
                #============guidedquant：optimize_module调用修改结束=====
