...
class PseudoQuantizedLinear(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Task-adaptive angle residuals (方案5)
        self.enable_task_adaptive = enable_task_adaptive
        if enable_task_adaptive:
            self.angles_residual_short = nn.Parameter(torch.zeros_like(angles_grouped))
            self.angles_residual_long = nn.Parameter(torch.zeros_like(angles_grouped))
        else:
            self.register_buffer("angles_residual_short", None)
            self.register_buffer("angles_residual_long", None)
        # Task type: 0=shared, 1=short, 2=long
        self.register_buffer("task_type_id", torch.tensor(0, dtype=torch.int32))
    ...
    def set_task_type(self, task_type: str) -> None:
        """切换当前任务类型：'shared' | 'short' | 'long'"""
        mapping = {"shared": 0, "short": 1, "long": 2}
        self.task_type_id = torch.tensor(
            mapping[task_type], dtype=torch.int32, device=self.weight.device
        )

    def _get_effective_angles(self) -> torch.Tensor:
        """根据当前任务类型返回有效的旋转角度。"""
        if not self.enable_task_adaptive:
            return self.angles_grouped
        tid = self.task_type_id.item()
        if tid == 1 and self.angles_residual_short is not None:
            return self.angles_grouped + self.angles_residual_short
        elif tid == 2 and self.angles_residual_long is not None:
            return self.angles_grouped + self.angles_residual_long
        return self.angles_grouped

    def _pseudo_quantize(self, weight: torch.Tensor) -> torch.Tensor:
        effective_angles = self._get_effective_angles()
        ...
        if self.pairs_grouped is not None:
            ...
            flipped_angles = torch.flip(effective_angles, dims=[0])
            weight = self.checkpointed(
                scaled_pairwise_rotation,
                weight,
                flipped_pairs,
    ...
    def set_optim_enabled(
        ...
        *,
        ...
        # Task-adaptive residuals follow the same 'angles' flag
        if self.enable_task_adaptive:
            if self.angles_residual_short is not None:
                self.angles_residual_short.requires_grad = angles
            if self.angles_residual_long is not None:
                self.angles_residual_long.requires_grad = angles
        if self.channel_scales is not None:
    ...
    def get_optim_params(self, name: str) -> list[nn.Parameter]:
        if name == "weight":
            ...
            params = []
            if self.angles_grouped is not None:
                params.append(self.angles_grouped)
            if self.enable_task_adaptive:
                if self.angles_residual_short is not None:
                    params.append(self.angles_residual_short)
                if self.angles_residual_long is not None:
                    params.append(self.angles_residual_long)
            return params
    ...
    @classmethod
    def from_state_dict(cls, state_dict: dict, task_type: str = "shared") -> "PseudoQuantizedLinear":
        ...
        # Detect whether state_dict contains task-adaptive params
        enable_ta = "angles_residual_short" in state_dict or "angles_residual_long" in state_dict
        ...
        qlinear = PseudoQuantizedLinear(
            ...
            enable_task_adaptive=enable_ta,
        )
        ...
        qlinear.load_state_dict(state_dict, strict=False)
        qlinear.set_task_type(task_type)
        return qlinear
