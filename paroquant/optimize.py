...
#============guidedquant：在optimize_module签名中增加saliency_weights参数和saliency_weighted_mse loss选项=====
def optimize_module(
    module: nn.Module,
    ...
    *,
    loss_fn: Literal["mse", "smooth_l1", "saliency_weighted_mse"],
    n_iter: int,
    ...
    #============guidedquant：增加saliency-weighted MSE损失函数的实现=====
    # 根据传入的 loss_fn 字符串选择对应的损失函数
    if loss_fn == "mse":
        loss = nn.MSELoss()
    elif loss_fn == "smooth_l1":
        loss = nn.SmoothL1Loss()
    elif loss_fn == "saliency_weighted_mse":
        if saliency_weights is None:
            raise ValueError(
                "saliency_weights must be provided when loss_fn='saliency_weighted_mse'"
            )
        # 将 saliency 移动到计算设备，并归一化到均值为1（保持loss量级稳定，兼容现有lr）
        sal = saliency_weights.to(next(module.parameters()).device)
        sal = sal / sal.mean()
        base_loss_fn = nn.MSELoss(reduction="none")

        def saliency_weighted_loss(target: torch.Tensor, pred: torch.Tensor) -> torch.Tensor:
            diff_sq = base_loss_fn(target, pred)  # element-wise squared error
            # 在最后一个维度（hidden dimension）应用 saliency 权重
            view_shape = [1] * (diff_sq.dim() - 1) + [-1]
            weighted = diff_sq * sal.view(*view_shape)
            return weighted.mean()

        loss = saliency_weighted_loss
    else:
        raise ValueError(f"Unsupported loss function: {loss_fn}")
    #============guidedquant：saliency-weighted MSE损失函数实现结束=====
