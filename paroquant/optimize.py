...
def get_random_rotation_pairs(
    ...
    metric: str = "l2",
    pivot_saliency: Optional[torch.Tensor] = None,
) -> list[tuple[int, int]]:
#============intactKV：增加 pivot_saliency 参数=====
    ...
    #============intactKV：基于 pivot_saliency 调整通道显著性分数=====
    # 如果提供了 pivot_saliency，则提升对 pivot tokens 敏感的通道的显著性分数，
    # 使得这些通道在贪心配对时获得更保守/保护性的配对策略。
    if pivot_saliency is not None and selection_mode == "greedy":
        assert pivot_saliency.shape[0] == channel_scores.numel(), \
            f"pivot_saliency shape {pivot_saliency.shape} mismatches with channel_scores {channel_scores.numel()}"
        pivot_saliency = pivot_saliency.view_as(channel_scores)
        # 将 pivot_saliency 归一化后作为乘性 boost
        pivot_saliency = pivot_saliency / (pivot_saliency.max() + 1e-8)
        channel_scores = channel_scores * (1.0 + pivot_saliency)
    #============intactKV：基于 pivot_saliency 调整通道显著性分数=====
...
#============intactKV：增加 pivot_token_mask 和 pivot_loss_weight 参数=====
def optimize_module(
    module: nn.Module,
    ...
    *,
    ...
    pivot_token_mask: Optional[torch.Tensor] = None,
    pivot_loss_weight: float = 0.0,
) -> None:
#============intactKV：增加 pivot_token_mask 和 pivot_loss_weight 参数=====
    ...
    #============intactKV：定义 Pivot-Aware Loss 辅助函数=====
    def compute_pivot_aware_loss(
        ...
        """计算 Pivot-Aware 加权损失。"""
        base_loss = loss(output_q, output_gt)
        if mask is None or weight <= 0.0:
            return base_loss
        # mask 形状: (batch, seq_len) 或 (seq_len,)
        # output 形状: (batch, seq_len, hidden_dim)
        if mask.dim() == 1:
            mask = mask.unsqueeze(0).expand(output_q.shape[0], -1)
        if mask.shape[0] != output_q.shape[0] or mask.shape[1] != output_q.shape[1]:
            # 如果 mask 维度不匹配（例如因为 batch 被分片），跳过 pivot loss
            return base_loss
        # 将 mask 扩展到 hidden_dim 维度
        mask_expanded = mask.unsqueeze(-1).float().to(output_q.device)
        pivot_mse = ((output_q - output_gt) ** 2 * mask_expanded).sum() / (mask_expanded.sum() + 1e-8)
        return base_loss + weight * pivot_mse
    #============intactKV：定义 Pivot-Aware Loss 辅助函数=====
    ...
    def loss_batches(
        ...
        for input_batch, output_batch in zip(input_batches, output_batches):
            ...
            #============intactKV：验证集也使用 Pivot-Aware Loss=====
            loss_value = compute_pivot_aware_loss(
                output_q, output_batch, pivot_token_mask, pivot_loss_weight
            )
            #============intactKV：验证集也使用 Pivot-Aware Loss=====
