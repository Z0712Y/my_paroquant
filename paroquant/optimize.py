...
def get_random_rotation_pairs(
    ...
    # 贪心/softmax 模式下预计算所有通道显著性；随机模式下保留原随机种子逻辑
    if selection_mode in ("greedy", "softmax"):
    ...
    for group_idx in range(group_num):
        ...
        if selection_mode == "greedy":
            ...
            pair_scores_t = torch.tensor(pair_scores, device=sensitivity_input.device, dtype=torch.float32)
            # 应用 temperature：T->0 接近贪心，T->∞ 接近均匀随机
            probs = torch.softmax(pair_scores_t / temperature, dim=0)
            # 无放回加权采样，得到候选对的排列顺序
            sampled_indices = torch.multinomial(probs, num_samples=len(candidate_pairs), replacement=False)
            sorted_pairs.append([candidate_pairs[idx] for idx in sampled_indices.cpu().tolist()])
        else:
