...
# cd /home/hhw/zy/paroquant-3 && ./run_4bit_with_log.sh

# ==================== 配置区域 ====================
# 修改以下变量即可更换模型或脚本
# 外部配置优先级 > 脚本内部配置，但只执行脚本时会安全地使用默认值。
SCRIPT="experiments/optimize/4bit2.sh"  # 要运行的脚本
MODEL="/home/hhw/zy/models/Qwen3-8B-Base"            # 模型路径
# GPU_ID="2"                             # 指定使用的 GPU

#============intactKV：提示用户启用 IntactKV 的方式=====
# 如需启用 Pivot-Aware Loss（IntactKV 优化阶段），
# 请编辑 experiments/optimize/4bit2.sh 中的以下变量：
#   PIVOT_AWARE_LOSS_FLAG="是"
...
if [ -z "$_IN_NOHUP" ]; then
    ...
    LOG_DIR="/home/hhw/zy/paroquant-3/experiments/optimize/logs/$(date +%Y%m%d_%H%M%S)"
    mkdir -p "$LOG_DIR"
