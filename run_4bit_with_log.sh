...
MODEL="/home/hhw/zy/models/Qwen3-14B"            # 模型路径（三合一方案使用 Qwen3-8B）
# GPU_ID="2"                             # 指定使用的 GPU
# ==================================================
...
if [ -z "$_IN_NOHUP" ]; then
    ...
    LOG_DIR="/home/hhw/zy/paroquant-4/experiments/optimize/logs/$(date +%Y%m%d_%H%M%S)"
    mkdir -p "$LOG_DIR"
    ...
    # 保存基本信息
    {
        ...
        echo "方案: ParoQuant-4 三合一（Prefix-Activation-Aware + Prefix-Stabilized + Task-Adaptive）"
    } > "$LOG_DIR/info.txt"
