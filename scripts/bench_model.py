...
#============intactKV：导入 IntactKV 工具=====
from paroquant.intactkv_utils import (
    compute_intactkv,
    inject_intactkv_to_static_cache,
)
...
# 模块3: 核心基准测试函数
# =============================================================================
#============intactKV：增加 fp16_model 和 use_intactkv 参数=====
@torch.no_grad()  # 装饰器方式禁用梯度计算
def benchmark(model, tokenizer, prefill_len, decode_len, past_kv, fp16_model=None, use_intactkv=False, intactkv_pivot_len=None):
#============intactKV：增加 fp16_model 和 use_intactkv 参数=====
    """
    ...
    # -------------------------------------------------------------------------
    #============intactKV：支持 IntactKV 注入=====
    if use_intactkv and fp16_model is not None:
        # 使用 FP16 模型预计算 prompt 的 KV cache
        pivot_len = intactkv_pivot_len or seq_len
        pivot_len = min(pivot_len, seq_len)
        key_caches, value_caches, _ = compute_intactkv(
            fp16_model, tokenizer, fixed_text * ((prefill_len // len(fixed_text)) + 1),
            device=0, dtype=torch.float16, max_length=seq_len,
        )
        # 重新编码确保文本一致
        full_text = tokenizer.decode(enc_full["input_ids"][0], skip_special_tokens=False)
        key_caches, value_caches, _ = compute_intactkv(
            fp16_model, tokenizer, full_text, device=0, dtype=torch.float16,
        )
        inject_intactkv_to_static_cache(
            key_caches, value_caches, past_kv, pivot_len=pivot_len
        )
        # 量化模型只处理最后一个 token
        last_token = enc_full["input_ids"][:, -1:].to(0)
        last_cache_position = torch.tensor([seq_len - 1], device=0)
        out = model(
        ...
        # decode 位置从 seq_len 开始（因为最后一个 token 已经处理过了）
        decode_position = torch.tensor([seq_len], device=0)
    else:
        # 将模型输入移动到GPU
...
# 模块4: 运行完整基准测试流程
# =============================================================================
#============intactKV：增加 use_intactkv 和 intactkv_pivot_len 参数=====
def run_benchmark(hf_path, prefill_len, decode_len, empty_model, max_new_tokens, use_intactkv=False, intactkv_pivot_len=None):
#============intactKV：增加 use_intactkv 和 intactkv_pivot_len 参数=====
    """
    ...
    #============intactKV：加载 FP16 参考模型=====
    fp16_model = None
    if use_intactkv and not empty_model:
        print("IntactKV: loading FP16 reference model for prefill...")
    ...
    benchmark(model, tokenizer, 2, 8, past_kv, fp16_model=fp16_model, use_intactkv=use_intactkv, intactkv_pivot_len=intactkv_pivot_len)
    ...
    # -------------------------------------------------------------------------
    # 再次warmup，这次使用编译后的函数
    benchmark(model, tokenizer, 16, 16, past_kv, fp16_model=fp16_model, use_intactkv=use_intactkv, intactkv_pivot_len=intactkv_pivot_len)
    # 执行正式的基准测试，获取吞吐量结果
    _, _, decode_tps = benchmark(model, tokenizer, prefill_len, decode_len, past_kv, fp16_model=fp16_model, use_intactkv=use_intactkv, intactkv_pivot_len=intactkv_pivot_len)
    # 打印最终的解码吞吐量结果
    print(
...
if __name__ == "__main__":
    ...
    #============intactKV：增加 IntactKV 命令行参数=====
    parser.add_argument(
        "--use-intactkv",
        action="store_true",
        help="Enable IntactKV: use FP16 model to precompute KV cache for pivot tokens",
    )
    parser.add_argument(
        "--intactkv-pivot-len",
        type=int,
        default=None,
        help="Number of initial tokens to protect with IntactKV (default: full prompt)",
    )
    #============intactKV：增加 IntactKV 命令行参数=====
    # 解析命令行参数
    ...
    run_benchmark(
        ...
        use_intactkv=cli_args.use_intactkv,
        intactkv_pivot_len=cli_args.intactkv_pivot_len,
    )
    #============intactKV：传递 IntactKV 参数=====
